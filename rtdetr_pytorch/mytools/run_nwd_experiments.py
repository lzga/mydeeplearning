...
def schedule_shutdown(delay_seconds: int = 60) -> None:
    ...
    try:
        if "windows" in system_name:
            print(f"将在 {delay_seconds} 秒后自动关机。若要取消，请在 CMD/PowerShell 执行：shutdown /a")
            ...
            print(f"将在约 {minutes} 分钟后自动关机。若要取消，请执行：sudo shutdown -c")
            subprocess.run(["shutdown", "-h", f"+{minutes}"], check=False)
        else:
            print(f"未识别的系统类型：{platform.system()}，未执行自动关机。")
    except Exception:
        print("自动关机命令执行失败，错误信息如下：")
        traceback.print_exc()
