...
from .torchvision_compat import datapoints
...
# Keep old YAML names while supporting newer torchvision API names.
_BaseToImageTensor = getattr(T, 'ToImageTensor', getattr(T, 'ToImage'))
@register
class ToImageTensor(_BaseToImageTensor):
    pass

_BaseConvertDtype = getattr(T, 'ConvertDtype', getattr(T, 'ConvertImageDtype'))
@register
class ConvertDtype(_BaseConvertDtype):
    def __init__(self, dtype=torch.float32, *args, **kwargs):
        try:
            super().__init__(dtype=dtype, *args, **kwargs)
        except TypeError:
            super().__init__(*args, **kwargs)

_BaseSanitizeBoundingBox = getattr(T, 'SanitizeBoundingBox', getattr(T, 'SanitizeBoundingBoxes'))
@register
...
class ConvertBox(T.Transform):
    ...
    def _transform(self, inpt: Any, params: Dict[str, Any]) -> Any:  
        if self.out_fmt:
            ...
            in_fmt = inpt.format.value.lower() if hasattr(inpt.format, 'value') else str(inpt.format).lower()
